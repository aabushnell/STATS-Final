...
write.csv(state_data_trans, file = "state_data.csv")
...
rt_words <- most_common_words(rt_14) %>%
  mutate(occurences = occurences / num_rows$`n()`) %>%
  inner_join(time_data, by = 'word') %>%
  rename('14' = occurences)
time_data <- rt_words
...
ggplot(time_data_trans, aes_string(x='Time', y=selected_word)) + geom_line() + geom_point()

write.csv(time_data_trans, file = "time_data.csv")
```
